...
int main(int argc, char *argv[])
{
	...
	for(int i = 6; i < ntokens; i+=6)
	{
		...
		outputcontent->open = atoi(token[i + 1]);
		outputcontent->high = atoi(token[i + 2]);
		outputcontent->low = atoi(token[i + 3]);
		outputcontent->close = atoi(token[i + 4]);
		outputcontent->volume = atoi(token[i + 5]);
		outputcontent++;
		//printf("%d..",i);
	}
